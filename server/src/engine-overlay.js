...
function OverlayEngine(options) {
    ...
    _.forEach(options.environment.overrides, (function(conf, name){
        ...
        if (!fs.existsSync("./server/src/modules/" + this.configuration.overrides[name].module.location)) {
            console.error("./server/src/modules/" + this.configuration.overrides[name].module.location + " does not exist!");
        } else {
